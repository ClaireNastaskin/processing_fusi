...
def get_sidecar(bmode_h5, pwd_h5, time_stamps, n_tc,
    ...
    sidecar = dict(
        ...
        Depth = [metadata_pwd['depth'][0].round(7) * 1000,
                 metadata_pwd['depth'][-1].round(7)] * 1000,
        Lateral = [metadata_pwd['lateral'][0].round(7) * 1000,
                   metadata_pwd['lateral'][-1].round(7) * 1000],
        ...
        VolumeTimingStart = acquire_start_datetime, # global time
...
def get_fusi_frames(power_doppler_path, power_doppler_df, frame_indices=-1):
    ...
    stacked_data = np.flip(stacked_data, axis=1)
...
def register_image_stack(image_stack):
    ...
    registered_array = np.stack([sitk.GetArrayFromImage(img)
                                 for img in registered_images], axis=2)
