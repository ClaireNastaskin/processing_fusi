...
def get_fusi_frames(power_doppler_path, power_doppler_df, frame_indices=-1):
    ...
    frames_data_replicated = stacked_data
...
def calculate_stimulus_events_caltech_daq(behavior_df):
    ...
    on_times = behavior_df[behavior_df['Event'] == 'pwm_enable']['timestamp'].reset_index(drop=True)
    off_times = behavior_df[behavior_df['Event'] == 'pwm_disable']['timestamp'].reset_index(drop=True)
    ...
    # Extract conditions and onset times
    stimulus_df = behavior_df[behavior_df['Event'] == 'pwm_enable']
    conditions = stimulus_df['Event'].tolist()
    onsets = stimulus_df['timestamp'].tolist()
