...
class DirectoryManager:
    def __init__(self, base_path, sequence):
        ...
        self.probe_event_file_path = (
            self.base_path / "streams" / "probe-event_stream.h5"
        )
        self.power_doppler_path = self.base_path / f"{self.sequence}_pwd.nii.gz"

    def list_acquisition_directories(self):
        acquisitions_path = self.base_path
...
def extract_task_events(task_event_stream):
    ...
    for i in range(len(timestamps)):
        data.append({"Event": events[i].decode("utf-8"), "Timestamp": timestamps[i]})
...
def extract_nilearn_compatible_events(behavior_df):
    ...
    on_times = behavior_df[behavior_df["Event"] == "stimulus_onset"][
        "Timestamp"
    ].reset_index(drop=True)
    off_times = behavior_df[behavior_df["Event"] == "stimulus_offset"][
        "Timestamp"
    ].reset_index(drop=True)
...
def load_fusi_frames(power_doppler_path, probe_events, frame_indices=-1):
    ...
    if frame_indices == -1:
        ...
        frame_index < 0 or frame_index >= len(probe_events)
    ...
    frames_data_replicated = np.flip(
        frames_data_replicated, axis=2
    )  # get in the right coordinate system
...
def process_power_doppler_files(
    power_doppler_path, num_tissue_components, probe_events
):
    import os
    import re
...
def register_image_stack(image_stack):
    sitk_images = [
        sitk.GetImageFromArray(image_stack[:, :, i])
...
def register_ants(image_stack):
    ...
    for i in range(1, image_stack.shape[2]):
        ...
        transform = (
            result["fwdtransforms"][0] if len(result["fwdtransforms"]) > 0 else None
        )
        if transform:
